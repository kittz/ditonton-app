...
class _MovieDetailPageState extends State<MovieDetailPage> {
  ...
  void initState() {
    ...
    Future.microtask(() {
      context.read<MovieDetailBloc>().add(
            MovieDetailEvent(widget.id),
          );
      context.read<MovieRecommendationsBloc>().add(
            MovieRecommendations(widget.id),
          );
      context.read<WatchlistMoviesBloc>().add(
            WatchlistMoviesStatus(widget.id),
          );
    });
  }
  ...
  Widget build(BuildContext context) {
    final isAddedWatchlist = context.select<WatchlistMoviesBloc, bool>((value) {
      if (value.state is WatchlistMoviesStatusState) {
        return (value.state as WatchlistMoviesStatusState).result;
      }
      return false;
    });
    return Scaffold(
      body: BlocBuilder<MovieDetailBloc, MoviesState>(
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                              ...
                              onPressed: () async {
                                if (!widget.isAddedWatchlist) {
                                  context
                                      ...
                                      .add(SaveWatchlistMovies(widget.movie));
                                } else {
                                  context
                                      .read<WatchlistMoviesBloc>()
                                      .add(RemoveWatchlistMovies(widget.movie));
                                }

                                final message = context.select<
                                    WatchlistMoviesBloc,
                                    String>((value) => (value.state
                                        is WatchlistMoviesStatusState)
                                    ? (value.state as WatchlistMoviesStatusState)
                                                .result ==
                                            false
                            ...
                            BlocBuilder<MovieRecommendationsBloc, MoviesState>(
                              builder: (context, state) {
